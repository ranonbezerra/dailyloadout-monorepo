...
if (failedReviews.length > 0) {
  console.log('Avoiding past review mistakes:');
...
SECURITY_RESULTS=$(npx claude-flow@v3alpha github review-security \
  --pr 123 \
  --files "$CHANGED_FILES" \
  --check "owasp,cve,secrets,permissions,ssti,jwt" \
  --suggest-fixes)
...
- [ ] Alembic migration has downgrade path
- [ ] Pydantic schemas validate all inputs
- [ ] Taskiq workers have proper error handling
- [ ] Jinja2 templates use SandboxedEnvironment (SSTI prevention)
- [ ] JWT secret not hardcoded; production guard present
- [ ] Test coverage >= 90%

## Web Review (packages/web)
- [ ] Mantine components follow design system
