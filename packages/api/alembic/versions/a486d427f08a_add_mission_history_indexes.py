...
def upgrade() -> None:
    ...
    Stats and mission-history filter ``user_id AND ended_at IS NOT NULL`` and
    ...
    """
    op.create_index(
        "idx_missions_user_ended_at",
        "missions",
        ["user_id", sa.literal_column("ended_at DESC")],
        unique=False,
    )
    op.create_index(
        "idx_missions_user_started_at",
        "missions",
...
def downgrade() -> None:
    """Drop the mission history indexes."""
    op.drop_index("idx_missions_user_started_at", table_name="missions")
    op.drop_index("idx_missions_user_ended_at", table_name="missions")
