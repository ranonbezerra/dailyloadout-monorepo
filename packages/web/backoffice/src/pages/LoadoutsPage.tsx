...
import type { LoadoutAction } from "../types/backoffice";
import { relativeTime } from "./shared";

const PAGE_SIZE = 20;
const ACTIONS: LoadoutAction[] = ["pending", "accepted", "rejected", "ignored"];
...
function LoadoutStat({ label, value }: { label: string; value: number }) {
	return (
		<Card padding="md">
...
export function LoadoutsPage() {
	...
	const { data, isLoading, isError } = useLoadouts({
		q: debounced || undefined,
		action: (action as LoadoutAction) ?? undefined,
		limit: PAGE_SIZE,
		...
		<Stack gap="lg">
			<div>
				...
				<Text c="dimmed" size="sm">
					Browse loadout suggestions across all users (read-only).
				</Text>
			</div>
			...
			{tallies.length > 0 && (
				<SimpleGrid cols={{ base: 2, xs: tallies.length > 4 ? 4 : tallies.length }}>
					{tallies.map((t) => (
						<LoadoutStat key={t.action} label={t.action} value={t.count} />
					))}
				</SimpleGrid>
			)}
			...
			<Card padding={0}>
				{isLoading ? (
					...
					<Center py="xl">
						<Text c="red">Failed to load loadouts.</Text>
					</Center>
				) : !data || data.items.length === 0 ? (
					<Center py="xl">
						<Stack align="center" gap={4}>
							<IconSearchOff size={28} color="var(--mantine-color-dimmed)" />
							<Text c="dimmed">No loadouts match.</Text>
						</Stack>
					</Center>
				) : (
			...
			<LoadoutDrawer publicId={viewing} onClose={() => setViewing(null)} />
		</Stack>
	);
}

function LoadoutDrawer({ publicId, onClose }: { publicId: string | null; onClose: () => void }) {
	const { data, isLoading } = useLoadout(publicId);
