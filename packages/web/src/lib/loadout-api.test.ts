...
describe("createLoadout", () => {
	it("calls POST /v1/loadouts with snake_cased body and defaults", async () => {
		const apiResponse = [
			{
				public_id: "lo1",
				library_entry: null,
		...
		const result = await createLoadout("chill", 60, "low");

		expect(mockApiFetch).toHaveBeenCalledWith("/v1/loadouts", {
			method: "POST",
			body: JSON.stringify({
				mood: "chill",
				available_minutes: 60,
	...
	it("passes custom count and context", async () => {
		...
		await createLoadout("focused", 120, "high", 3, "Want something story-driven");
	...
	it("converts nested library_entry to camelCase", async () => {
		...
		const result = await createLoadout("energetic", 90, "high");
...
describe("acceptLoadout", () => {
	it("calls POST /v1/loadouts/:publicId/accept", async () => {
		const apiResponse = {
			public_id: "lo1",
			library_entry: null,
			mood: "chill",
		...
		const result = await acceptLoadout("lo1");

		expect(mockApiFetch).toHaveBeenCalledWith("/v1/loadouts/lo1/accept", {
			method: "POST",
		});
...
describe("rejectLoadout", () => {
	it("calls POST /v1/loadouts/:publicId/reject", async () => {
		const apiResponse = {
			public_id: "lo1",
			library_entry: null,
			mood: "chill",
		...
		const result = await rejectLoadout("lo1");

		expect(mockApiFetch).toHaveBeenCalledWith("/v1/loadouts/lo1/reject", {
			method: "POST",
		});
...
describe("listLoadouts", () => {
	it("calls GET /v1/loadouts with no params when none given", async () => {
		const apiResponse = { items: [], total: 0 };
		mockApiFetch.mockResolvedValueOnce(apiResponse);

		const result = await listLoadouts();
	...
	it("appends query params for limit and offset", async () => {
		...
		await listLoadouts({ limit: 10, offset: 20 });
...
describe("getLatestLoadout", () => {
	it("calls GET /v1/loadouts/latest and returns camelCased data", async () => {
		const apiResponse = {
			public_id: "lo1",
			library_entry: null,
			mood: "chill",
		...
		const result = await getLatestLoadout();

		expect(mockApiFetch).toHaveBeenCalledWith("/v1/loadouts/latest");
